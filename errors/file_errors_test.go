...
func TestFileErrors(t *testing.T) {
	t.Run("FileNotFoundError", func(t *testing.T) {
		...
		assert.Equal(t, fmt.Sprintf("The FileEntity was not found on request: %s", requestString), err.Error())
	...
	t.Run("FileNotFoundByIdError", func(t *testing.T) {
		...
		assert.Equal(t, fmt.Sprintf("The FileEntity was not found on request: Id = %s", fileId), err.Error())
		assert.Equal(t, http.StatusNotFound, err.Code())
	})
}
